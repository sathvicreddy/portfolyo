...
router.post('/contact', async (req, res) => {
  try {
    ...
    if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
      ...
      console.log('📬 Simulated Email Received:', { name, email, message });
    ...
    // Setup email data
    const mailOptions = {
      from: `"${name}" <${process.env.EMAIL_USER}>`, // Send via authenticated user
      replyTo: email,
      to: 'satvicreddyc@gmail.com', // 🎯 The target destination
    ...
    console.error('❌ Email framing error:', err.message);
    res.status(500).json({ msg: 'Server Error: Failed to send email', error: err.message });
  }
});
